"""
Preprocessing script for HER2 Breast Tumor F1 sample.

Data: ST (Spatial Transcriptomics) single-modality RNA
    - 691 spots x 14,860 genes
    - 4 labels (3 tissue types + undetermined)
    - Clustering uses all 691 spots; evaluation excludes undetermined (660)

Pipeline (single modality RNA, same as H1/A1/B1/C1/D1/E1):
...
Data source: data/raw/F1_sample/
Output: data/processed/her2_breast_f1/
"""
...
def main():
    print("=" * 80)
    print("Preprocessing HER2 Breast Tumor F1 Sample")
    print("=" * 80)
    print(f"Start time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

    raw_dir = os.path.join(PROJECT_ROOT, 'data', 'raw', 'F1_sample')
    output_dir = os.path.join(PROJECT_ROOT, 'data', 'processed', 'her2_breast_f1')
    ...
    counts_df = pd.read_csv(os.path.join(raw_dir, 'F1.tsv.gz'), sep='\t', index_col=0)
    ...
    labels_df = pd.read_csv(os.path.join(raw_dir, 'F1_labeled_coordinates.tsv'), sep='\t')
    print(f"  Labels file: {labels_df.shape[0]} rows")

    selection_df = pd.read_csv(os.path.join(raw_dir, 'F1_selection.tsv'), sep='\t')
    print(f"  Selection file: {selection_df.shape[0]} spots")

    # Build mapping: count_matrix_index -> (new_x, new_y, sel_pixel_x, sel_pixel_y)
    sel_map = {}
