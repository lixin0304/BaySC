"""
HER2 Breast C1: Compute all metrics (including spARI) with locked parameters.
Save clustering results (GT + prediction) and append metrics to CSV.

Params: K=3: lam=15, th=400, t0=8, gamma=3, ARI=0.5553
"""
...
def main():
    ...
    data_dir = os.path.join(PROJECT_ROOT, 'data', 'processed', 'her2_breast_c1')
    output_dir = os.path.join(PROJECT_ROOT, 'outputs', 'HER2_Breast_C1')
    ...
    for i, lbl in enumerate(unique_gt):
        ...
        ax.scatter(pos_eval[mask, 0], pos_eval[mask, 1],
                   c=[colors_gt[i]], s=12, alpha=0.8, label=name)
    ...
    for i, lbl in enumerate(unique_pred):
        ...
        size = mask.sum()
        ax.scatter(pos_eval[mask, 0], pos_eval[mask, 1],
                   c=[colors_pred[i]], s=12, alpha=0.8, label=f'C{lbl} ({size})')
