"""
HER2 Breast B1: Compute all metrics (including spARI) with locked parameters.
Save clustering results (GT + prediction) and append metrics to CSV.

Params: K=4: lam=30, th=350, t0=12, gamma=2, ARI=0.4146
"""
...
def main():
    ...
    data_dir = os.path.join(PROJECT_ROOT, 'data', 'processed', 'her2_breast_b1')
    output_dir = os.path.join(PROJECT_ROOT, 'outputs', 'HER2_Breast_B1')
    ...
    for i, lbl in enumerate(unique_gt):
        ...
        ax.scatter(pos_eval[mask, 0], pos_eval[mask, 1],
                   c=[colors_gt[i]], s=10, alpha=0.8, label=name)
    ...
    for i, lbl in enumerate(unique_pred):
        ...
        size = mask.sum()
        ax.scatter(pos_eval[mask, 0], pos_eval[mask, 1],
                   c=[colors_pred[i]], s=10, alpha=0.8, label=f'C{lbl} ({size})')
