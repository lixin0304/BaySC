"""
Preprocessing script for HER2 Breast Tumor B1 sample.

Data: ST (Spatial Transcriptomics) single-modality RNA
    - 295 spots x 15,108 genes
    - 5 labels (4 tissue types + undetermined)
    - Clustering uses all 295 spots; evaluation excludes undetermined (269)

Pipeline (single modality RNA, same as H1/A1):
...
Data source: data/raw/B1_sample/
    - B1.tsv.gz: count matrix
    - B1_labeled_coordinates.tsv: spot labels + pixel coordinates
    - B1_selection.tsv: spot selection with pixel coordinates

Output: data/processed/her2_breast_b1/
"""
...
def main():
    print("=" * 80)
    print("Preprocessing HER2 Breast Tumor B1 Sample")
    print("=" * 80)
    print(f"Start time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

    raw_dir = os.path.join(PROJECT_ROOT, 'data', 'raw', 'B1_sample')
    output_dir = os.path.join(PROJECT_ROOT, 'data', 'processed', 'her2_breast_b1')
    ...
    counts_df = pd.read_csv(os.path.join(raw_dir, 'B1.tsv.gz'), sep='\t', index_col=0)
    ...
    labels_df = pd.read_csv(os.path.join(raw_dir, 'B1_labeled_coordinates.tsv'), sep='\t')
    print(f"  Labels file: {labels_df.shape[0]} rows")

    selection_df = pd.read_csv(os.path.join(raw_dir, 'B1_selection.tsv'), sep='\t')
