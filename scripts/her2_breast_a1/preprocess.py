"""
Preprocessing script for HER2 Breast Tumor A1 sample.

Data: ST (Spatial Transcriptomics) single-modality RNA
    - 346 spots x 15,044 genes
    - 6 labels (5 tissue types + undetermined)
    - Clustering uses all 346 spots; evaluation excludes undetermined

Pipeline (single modality RNA, same as HER2 H1):
    ...
    Step 6: PCA 30
    Step 7: L1 normalization (for rna_embedding.npy)
    Step 7b: Also save Z-score version for L2+cosine similarity

Data source: data/raw/A1_sample/
    - A1.tsv.gz: count matrix (346 spots x 15,044 genes)
    - A1_labeled_coordinates.tsv: spot labels + pixel coordinates
    - A1_selection.tsv: spot selection with pixel coordinates
    - A1-cluster-annotation.tsv.gz: cluster annotations

Output: data/processed/her2_breast_a1/
"""
...
def main():
    print("=" * 80)
    print("Preprocessing HER2 Breast Tumor A1 Sample")
    print("=" * 80)
    print(f"Start time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

    raw_dir = os.path.join(PROJECT_ROOT, 'data', 'raw', 'A1_sample')
    output_dir = os.path.join(PROJECT_ROOT, 'data', 'processed', 'her2_breast_a1')
    ...
    counts_df = pd.read_csv(os.path.join(raw_dir, 'A1.tsv.gz'), sep='\t', index_col=0)
    ...
    labels_df = pd.read_csv(os.path.join(raw_dir, 'A1_labeled_coordinates.tsv'), sep='\t')
    print(f"  Labels file: {labels_df.shape[0]} rows")

    # A1_selection.tsv: array coords (x, y) -> adjusted coords (new_x, new_y) -> pixel coords
    # A1_labeled_coordinates.tsv: (x, y) match selection's (new_x, new_y)
    selection_df = pd.read_csv(os.path.join(raw_dir, 'A1_selection.tsv'), sep='\t')
    ...
    E_rna_l1 = normalize(rna_pca, norm='l1', axis=1).astype(np.float32)
    print(f"    Final shape (L1): {E_rna_l1.shape}")
    print(f"    Range: [{E_rna_l1.min():.4f}, {E_rna_l1.max():.4f}]")

    # Step 7b: Z-score version (for L2+cosine similarity)
    print("  Step 7b: Z-score PCA (for L2+cosine)")
    E_rna_zscore = rna_pca.astype(np.float32)
    print(f"    Final shape (zscore): {E_rna_zscore.shape}")

    # ========================================================================
    # [4/4] Save
    # ========================================================================
    print("\n[4/4] Saving processed data...")
    np.save(os.path.join(output_dir, 'rna_embedding.npy'), E_rna_l1)
    np.save(os.path.join(output_dir, 'rna_embedding_pca30_zscore.npy'), E_rna_zscore)
    ...
    metadata = {
        ...
        'n_eval_spots': n_eval,
        'n_genes_raw': counts_df.shape[1],
        'n_genes_filtered': adata.n_vars,
        'n_hvg': n_hvg_actual,
        'n_pca': n_components,
        'n_labels_all': len(unique_labels),
        'n_labels_eval': len(unique_eval),
    }
    np.savez(os.path.join(output_dir, 'metadata.npz'), **metadata)

    print(f"\nSaved files:")
    print(f"  - rna_embedding.npy ({E_rna_l1.shape}) [L1 norm]")
    print(f"  - rna_embedding_pca30_zscore.npy ({E_rna_zscore.shape}) [Z-score]")
