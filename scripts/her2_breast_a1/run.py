"""
HER2 Breast A1: Compute all metrics (including spARI) with locked parameters.
Save clustering results (GT + prediction) and append metrics to CSV.

Params: K=5: lam=20, th=400, t0=12, gamma=5, ARI=0.2722
"""
...
def main():
    ...
    data_dir = os.path.join(PROJECT_ROOT, 'data', 'processed', 'her2_breast_a1')
    output_dir = os.path.join(PROJECT_ROOT, 'outputs', 'HER2_Breast_A1')
    ...
    print(f"Label names (all): {list(label_names_all)}")
    ...
    # Save clustering results (GT + prediction for all cells)
    ...
    cmap = plt.cm.get_cmap('tab10', max(len(unique_gt), len(label_names_eval)))
    for i, lbl in enumerate(unique_gt):
        ...
        ax.scatter(pos_eval[mask, 0], pos_eval[mask, 1],
                   c=[cmap(i)], s=8, alpha=0.8, label=name)
    ...
    cmap2 = plt.cm.get_cmap('tab10', max(len(unique_pred), 10))
    for i, lbl in enumerate(unique_pred):
        mask = z_eval == lbl
        size = mask.sum()
        ax.scatter(pos_eval[mask, 0], pos_eval[mask, 1],
                   c=[cmap2(i)], s=8, alpha=0.8, label=f'C{lbl} ({size})')
