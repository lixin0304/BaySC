"""
Preprocessing script for HER2 Breast Tumor G2 sample.

Data: ST (Spatial Transcriptomics) single-modality RNA
    - 467 spots x 15,257 genes
    - 7 labels (6 tissue types + undetermined)
    - Clustering uses all 467 spots; evaluation excludes undetermined (410)

Pipeline (single modality RNA, same as H1/A1/B1/C1/D1/E1/F1):
...
Data source: data/raw/G2_sample/
Output: data/processed/her2_breast_g2/
"""
...
def main():
    print("=" * 80)
    print("Preprocessing HER2 Breast Tumor G2 Sample")
    print("=" * 80)
    print(f"Start time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

    raw_dir = os.path.join(PROJECT_ROOT, 'data', 'raw', 'G2_sample')
    output_dir = os.path.join(PROJECT_ROOT, 'data', 'processed', 'her2_breast_g2')
    ...
    counts_df = pd.read_csv(os.path.join(raw_dir, 'G2.tsv.gz'), sep='\t', index_col=0)
    ...
    labels_df = pd.read_csv(os.path.join(raw_dir, 'G2_labeled_coordinates.tsv'), sep='\t')
    print(f"  Labels file: {labels_df.shape[0]} rows")

    selection_df = pd.read_csv(os.path.join(raw_dir, 'G2_selection.tsv'), sep='\t')
