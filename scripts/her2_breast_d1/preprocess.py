"""
Preprocessing script for HER2 Breast Tumor D1 sample.

Data: ST (Spatial Transcriptomics) single-modality RNA
    - 306 spots x 15,660 genes
    - 4 labels (3 tissue types + undetermined)
    - Clustering uses all 306 spots; evaluation excludes undetermined (258)

Pipeline (single modality RNA, same as H1/A1/B1/C1):
...
Data source: data/raw/D1_sample/
Output: data/processed/her2_breast_d1/
"""
...
def main():
    print("=" * 80)
    print("Preprocessing HER2 Breast Tumor D1 Sample")
    print("=" * 80)
    print(f"Start time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

    raw_dir = os.path.join(PROJECT_ROOT, 'data', 'raw', 'D1_sample')
    output_dir = os.path.join(PROJECT_ROOT, 'data', 'processed', 'her2_breast_d1')
    ...
    counts_df = pd.read_csv(os.path.join(raw_dir, 'D1.tsv.gz'), sep='\t', index_col=0)
    ...
    labels_df = pd.read_csv(os.path.join(raw_dir, 'D1_labeled_coordinates.tsv'), sep='\t')
    print(f"  Labels file: {labels_df.shape[0]} rows")

    selection_df = pd.read_csv(os.path.join(raw_dir, 'D1_selection.tsv'), sep='\t')
